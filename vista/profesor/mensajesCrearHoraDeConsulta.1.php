...
//ok
if(isset($_SESSION['creacionExitosa'])){
...
//error
if(isset($_SESSION['falloComprobacion'])){
...
//mesas
if(isset($_SESSION['igualMesa'])&&(isset($_SESSION['falloComprobacion']))){
    if($_SESSION["igualMesa"]&&(!$_SESSION['falloComprobacion'])){
...
$mostrarHoraiosMesas=false;
if(isset($_SESSION['falloComprobacion'])&& isset($_SESSION['igualMesa'])&& isset($_SESSION['horariosdeMesasAagregar']) ){
    $mostrarHoraiosMesas=(!$_SESSION['falloComprobacion'])&&($_SESSION['igualMesa'])&&($_SESSION['horariosdeMesasAagregar']);
}
if(isset($_POST['SoloMesas'])){
    $mostrarHoraiosMesas=true;
}
...
foreach ($cargar as $horario) {
   ...
   elseif($horario->getsemestre()==32){
     if($horario->getn()==2){
         ...
         array_push($horariosdeMesasAagregar,22);
       }else{
         $DM2S1=$horario->getdia()->getid_dia();
         $HM2S1=date("H", strtotime( $horario->gethora()));
         $MM2S1=date("i", strtotime( $horario->gethora()));
         array_push($horariosdeMesasAagregar,21);
       }
}
}
//cargar que mesas se agregar en caso de ir por el camino de cambiar solo horarios de mesas para enviarlo al crear.
if(!isset($_SESSION['horariosdeMesasAagregar'])){
    $_SESSION['horariosdeMesasAagregar']=$horariosdeMesasAagregar;
}
}
